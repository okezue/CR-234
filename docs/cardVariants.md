...
Royal Ghost Evolution creates two real Souldier troops when he becomes visible. Tests exercise third-play evolution, repeated reveals, independent attacks and idle disappearance at levels 11 and 16. Cloning him now produces a regular Royal Ghost without the Souldier-summoning component. Ghost clones own their invisibility timer; cloned Souldiers and other stealth-bearing troops have independent idle timers, preserving their existing elapsed values. Cloning and clone attacks no longer mutate the original's Stealth/Fade timers or Ghost summon latch; other component objects remain shared. Fresh Ghost stealth initialization and preservation of other elapsed timers are implementation choices, not video-validated behavior. Exact clone invisibility initialization and mixed-level Clone scaling remain unverified. Souldier deployment-blast placement and targeting still need stronger validation; the current shared spawn-damage implementation is not a claim of complete fidelity.

A bounded diagnostic characterizes the unchanged reveal blast as one pulse around the parent Ghost, using the child's inherited 1.2-tile attack range. It damages nearby air units and ordinary buildings but omits Crown Towers. In synthetic full-tick controls, the revealing attack occurs at 0.60 seconds and the pulse at 0.65 seconds, before either child is placed. A selected Knight 1.5 tiles from the Ghost receives the ordinary attack but misses the pulse, while a closer air unit receives it. These controls establish simulator behavior, not physical correctness.
