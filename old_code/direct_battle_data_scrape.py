...
async def scrape(page: Page) -> list[dict]:
    ...
    for i in range(count):
        ...
        # Wait for the markers div to load
        try:
            await page.wait_for_selector(".markers", timeout=10000)
        except Exception as e:
            print(f"Failed to load markers for battle {i}: {e}")
            continue

        # Scrape the markers
        markers = page.locator(".markers .marker")
        ...
        for j in range(marker_count):
            ...
            card = await marker.get_attribute("data-c")
            time = await marker.get_attribute("data-t")
            index = await marker.get_attribute("data-i")
            side = await marker.get_attribute("data-s")
            level_span = marker.locator("span")
            level = await level_span.text_content()
        ...
        await page.keyboard.press("Escape")
        await page.wait_for_timeout(500)  # Wait a bit for popup to close

        # Add battle info to each placement
    ...
    print("Finished scraping. Collected placements:", len(battle_data))
    return battle_data
