...
def train(horde,traj_csv,episodes=7000,team='blue',log_interval=20,save_path=None,save_interval=100):
    import pandas as pd
...
def main():
    ...
    ap.add_argument('--episodes',type=int,default=7000)
    ap.add_argument('--save',default='checkpoints/horde_v5.pt')
    ap.add_argument('--num-cards',type=int,default=180)
    ap.add_argument('--hs',type=int,default=192)
    args=ap.parse_args()
    dev=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f'Device: {dev}',flush=True)
    horde=build_v5_horde(num_cards=args.num_cards,hs=args.hs,device=dev)
    horde.to(dev)
    nd=len(horde.td_demons)+len(horde.cql_demons)+len(horde.awr_demons)
    print(f'HordeV5: {nd} demons, hs={args.hs}',flush=True)
    csv=args.traj_csv
    if csv is None:
