...
def build_card_vocab(traj_csv):
    ...
    vocab={c:i+1 for i,c in enumerate(cards)}
    return vocab

def train(horde,traj_csv,card_vocab,episodes=15000,team='blue',
    ...
    for ep in range(episodes):
        ...
        for ri in range(1,len(grp)):
            ...
            cumulants['dmg_efficiency'].append(min(opp_dmg/300.0,1.0))
            cumulants['placement_x'].append(rx/18.0)
            cumulants['placement_y'].append(ry/32.0)
            cumulants['win_pred'].append(win_label)
            binary_targets['will_damage'].append(float(opp_dmg>50))
            binary_targets['will_lose_tower'].append(float(team_dmg>50))
            binary_targets['winning'].append(float(win_label>0.5))
            awr_rewards['awr_offense'].append(opp_dmg/1000.0+float(g.players[team].crowns>0)*0.5)
            awr_rewards['awr_defense'].append(-team_dmg/1000.0)
            prev_feat=curr_feat;prev_thp=curr_thp
            if g.ended:break
        if len(feats)<2:continue
        losses=horde.train_episode(feats,deltas,card_idxs,cumulants,binary_targets,awr_rewards,actions)
...
def main():
    ...
    ap.add_argument('--save',default='checkpoints/horde_v8.pt')
    ...
    csv=args.traj_csv
    if csv is None:
        csv=str(Path(__file__).resolve().parents[2]/'data'/'ready_data'/'traj_full.csv')
    print(f'Building card vocab...',flush=True)
    vocab=build_card_vocab(csv)
    num_cards=len(vocab)+1
    horde=build_v8_horde(num_cards=num_cards,hs=args.hs,device=dev)
    horde.to(dev)
    nd=len(horde.td_demons)+len(horde.binary_demons)+len(horde.awr_demons)
    print(f'HordeV8: {nd} demons, hs={args.hs}, {num_cards} cards',flush=True)
