...
class Game:
    ...
    def _do_attack(self,tr,tgt,shot=None):
        ...
        for c in getattr(tr,'components',[]):
            if shot is None or not isinstance(c,(Recoil,EvoRoyalGiant)):c.on_attack(tr,tgt,self)
        for c in getattr(tgt,'components',[]):
    ...
    def _fire(self,tr,tgt):
        ...
        # Recoil follows release; the projectile starts at the pre-recoil position. The evolved Royal Giant's recoil shockwave is
        # generated by the shot as well, not by the cannonball landing.
        for c in getattr(tr,'components',[]):
    ...
    def _proc_troops(self):
        for tm in ('blue','red'):
            ...
            for tr in p.troops:
                ...
                if not tr.alive or has(tr,'burrowed') or has(tr,'deploying'):continue
