...
def t_goblin_demolisher_rocket_ride():
    ...
    b=mk_card('cannon',11,'red',9,20);g.deploy('red',b);d,=_dummies(g,(10.5,19),hp=5000);bh=b.hp
    gd.hp=int(gd.max_hp*0.4);g.run(0.1)
    assert gd.targets==['Buildings'] and abs(gd.spd-2.4)<0.01 and gd.is_suicide
    g.run(6)
    assert not gd.alive and bh-b.hp>=404 and 5000-d.hp==404 and d.x>10.5,f"{b.hp} {d.hp} {d.x}"
    return f"Goblin Demolisher below 50% rides the rocket to the cannon and explodes for 404 in 2.5 tiles with knockback (dummy x {d.x:.1f})"
